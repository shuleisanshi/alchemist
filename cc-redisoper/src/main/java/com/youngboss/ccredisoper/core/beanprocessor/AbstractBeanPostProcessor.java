...
import com.youngboss.ccredisoper.core.GenericRedisoper;
import com.youngboss.ccredisoper.core.RedisoperAware;
import com.youngboss.ccredisoper.core.metadata.index.EntityIndexEntry;
import com.youngboss.ccredisoper.core.metadata.EntityMetadata;
import com.youngboss.ccredisoper.core.metadata.index.IndexReader;
import com.youngboss.ccredisoper.core.metadata.annotation.CcRedisIndex;
import com.youngboss.ccredisoper.core.metadata.annotation.CcRedisPrimaryKey;
import com.youngboss.ccredisoper.core.metadata.index.EntityIndex;
import com.youngboss.ccredisoper.core.metadata.index.EntitySingleIndex;
import com.youngboss.ccredisoper.core.metadata.index.EntityUnionIndex;
import com.youngboss.ccredisoper.core.metadata.index.IndexWriter;
...
public abstract class AbstractBeanPostProcessor implements BeanPostProcessor, ApplicationContextAware {
	...
	public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
		if(bean instanceof RedisoperAware){
			...
			injectCcRedisoper((RedisoperAware) bean, entityClass);
		}
		return bean;
	}
	...
	private void injectCcRedisoper(RedisoperAware bean, Class entityClass) {
		...
		for (Field declaredField : entityClass.getDeclaredFields()) {
			...
			if (declaredField.isAnnotationPresent(CcRedisPrimaryKey.class)) {
				CcRedisPrimaryKey ccRedisPrimaryKey = declaredField.getAnnotation(CcRedisPrimaryKey.class);
				EntityIndexEntry primaryKey = new EntityIndexEntry();
				primaryKey.setIndexField(fieldName)
						  .setIndexFieldCap(fieldNameCap)
						  .setOrder(ccRedisPrimaryKey.order());
				handlerReaderAndWriter(primaryKey, entityClass, fieldName);
				primaryKeyList.add(primaryKey);
			}
			if (declaredField.isAnnotationPresent(CcRedisIndex.class)) {
				CcRedisIndex ccRedisIndex = declaredField.getAnnotation(CcRedisIndex.class);
				EntityIndexEntry indexEntry = new EntityIndexEntry();
				indexEntry.setUnique(ccRedisIndex.unique())
						  .setOrder(ccRedisIndex.order())
						  .setIndexField(fieldName)
						  .setIndexFieldCap(fieldNameCap);
				handlerReaderAndWriter(indexEntry, entityClass, fieldName);
				indexMap.compute(ccRedisIndex.name(), (s, entityIndexEntries) -> {
