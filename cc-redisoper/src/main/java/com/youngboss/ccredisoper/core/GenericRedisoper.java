...
import static com.youngboss.ccutil.Pager.of;
...
public class GenericRedisoper<T> {
	...
	public void batchSet2Redis(List<T> entities, boolean enableTransaction) {
		getRedisTemplate(enableTransaction).execute((RedisCallback<Void>) connection -> {
			doInPipeline(connection, conn -> of(entities, MAX_REDIS_PAGE).doConsume(e -> batchSet2RedisInner(conn, e)));
			return null;
		});
	}
	...
	public void batchDeleteInRedis(List<T> entities, boolean enableTransaction) {
		getRedisTemplate(enableTransaction).execute((RedisCallback<Void>) connection -> {
			doInPipeline(connection, conn -> of(entities, MAX_REDIS_PAGE).doConsume(e -> batchDeleteInRedisInner(conn, e)));
			return null;
		});
	}
